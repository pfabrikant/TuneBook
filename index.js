...
app.get('/friendshipStatus/:id', (req,res)=>{
    db.getFriendshipStatus(req.params.id, req.session.loginId).then(({rows})=>{
        if(rows.length==0){
        ...
        } else if (rows[0].sender_id==req.session.loginId && !rows[0].accepted){
...
app.post('/register',(req,res)=>{
    bc.getHash(req.body.password).then((hash) => {
        return db.insertUser(req.body.first, req.body.last, req.body.email, hash);
    }).then((id) => {
...
app.post('/uploadAvatar', uploader.single('file'), s3.uploadS3, (req, res) => {
    if (req.file) {
        ...
        db.uploadAvatar(req.session.loginId, url).then(()=>{
            res.json({
                success:true,
                avatarUrl:url
            });
...
app.post('/updateProfile', (req,res)=>{
    db.updateProfile(req.session.loginId, Object.keys(req.body)[0], Object.values(req.body)[0]).then(()=>{
...
app.post('/searchUsers', (req,res)=>{
    db.searchUsers(req.body.searchKey).then(({rows})=>{
        res.json(rows);
    }).catch(err=>{console.log('Error in POST / searchUsers route: ',err.message);
        res.json({error:true});
    });
});
app.post('/searchInstrument', (req,res)=>{
    db.searchInstrument(req.body.searchKey).then(({rows})=>{
        res.json(rows);
    }).catch(err=>{console.log('Error in POST / searchInstrument route: ',err.message);
        res.json({error:true});
    });
});
app.post('/searchBand', (req,res)=>{
    db.searchBand(req.body.searchKey).then(({rows})=>{
        res.json(rows);
    }).catch(err=>{console.log('Error in POST / searchBand route: ',err.message);
        res.json({error:true});
    });
});
...
app.post('/friendshipStatus', (req,res)=>{
    if (req.body.buttonMode=="Send Friendship Request"){
        db.insertFriendship(req.session.loginId,req.body.id).then(()=>{
            res.json({buttonMode:"Cancel Friendship Request"});
        ...
        db.confirmFriendship(req.body.id, req.session.loginId).then(()=>{
            res.json({buttonMode:"End Friendship"});
        ...
        db.cancelFriendshipRequest(req.session.loginId, req.body.id).then(()=>{
            res.json({buttonMode:"Send Friendship Request"});
        }).catch(err=>console.log("Error in POST /friendshipStatus cancelFriendshipRequest function: ",err.message));
    } else {
        db.deleteFriendship(req.session.loginId,req.body.id).then(()=>{
            res.json({buttonMode:"Send Friendship Request"});
...
io.on('connection', async function(socket) {
    ...
    const usefulUserInformation = db.getUserInfo(socket.request.session.loginId);
    const chatMessages= db.getChatMessages();
    const onlineUsersPromise = db.getUsersInfo(arrOfOnlineIds);
    const  privateMessages = db.getPrivateMessages(socket.request.session.loginId);
    const friends = db.getFriends(socket.request.session.loginId);

    var promiseAllResults;
    try {
        promiseAllResults = await Promise.all([usefulUserInformation, chatMessages, onlineUsersPromise, privateMessages, friends]);
    } catch (err) {
    ...
    if (onlineUsers[socket.request.session.loginId].length==1){
        io.sockets.sockets[socket.id].broadcast.emit("newOnlineUser", {id:socket.request.session.loginId, avatarurl:socket.request.avatarUrl, first:socket.request.first, last:socket.request.last });
    }
    ...
    socket.on("messageSent",(text)=>{
        db.addChatMessage(socket.request.session.loginId, text).then(({rows})=>{
            io.sockets.emit("newMessageAvailable",{message:text, messageid:rows[0].messageid, usersid:rows[0].usersid, created_at:rows[0].created_at, first: socket.request.first, last: socket.request.last, avatarurl: socket.request.avatarUrl});
        });
    });

    // listen for new private message
    socket.on("newPm", obj => {
        db.addPrivateMessage(obj.receiver_id, obj.sender_id, obj.message).then(({rows})=>{
            //make a list of all open sockets that belong to both sender and to receiver
            let arrOfSockets=[];

            if (onlineUsers[obj.receiver_id] && onlineUsers[obj.receiver_id].length>0){
                onlineUsers[obj.receiver_id].forEach(socketId=>arrOfSockets.push(socketId));
            } else {
                db.pmUnseen(rows.id, false);
            }
            if (onlineUsers[obj.sender_id] && onlineUsers[obj.sender_id].length>0) {
                onlineUsers[obj.sender_id].forEach(socketId=>arrOfSockets.push(socketId));
            }

            //emitting to this sockets
            arrOfSockets.forEach(socketId=>io.to(socketId).emit("updateNewPm",{receiver_id:obj.receiver_id, sender_id:obj.sender_id, message:obj.message, id:rows[0].id, created_at:rows[0].created_at, avatarurl:obj.avatarurl, first:obj.first, last:obj.last}));
        });
    ...
    socket.on('disconnect', function() {
        onlineUsers[socket.request.session.loginId] = onlineUsers[socket.request.session.loginId].filter(socketid => socketid!=socket.id);

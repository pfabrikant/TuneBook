...
var db = spicedPg(process.env.DATABASE_URL || 'postgres:postgres:postgres@localhost:5432/tunebook');
...
exports.getRecentUsers = function () {
    return db.query('SELECT id, first, last, avatarUrl FROM users ORDER BY id DESC LIMIT 3');
};
exports.searchUsers = function (key) {
    return db.query('SELECT id, first, last, avatarUrl FROM users WHERE first ILIKE $1 OR last ILIKE $1',[key + '%']);
};
exports.searchInstrument = function (key) {
    return db.query('SELECT id, first, last, avatarUrl FROM users WHERE instrument ILIKE $1',['%'+ key + '%']);
};

exports.searchBand = function (key) {
    return db.query('SELECT id, first, last, avatarUrl FROM users WHERE band ILIKE $1',['%'+ key + '%']);
};
exports.getFriendshipStatus = function (owner_id,viewer_id){
    return db.query('SELECT * FROM friendships WHERE (sender_id=$1 AND receiver_id=$2) OR (sender_id=$2 AND receiver_id=$1)',[owner_id, viewer_id]);
};
exports.insertFriendship = function (sender,receiver){
    return db.query(`INSERT INTO friendships (sender_id,receiver_id) VALUES (${sender}, ${receiver})`);
};
exports.confirmFriendship = function (sender, receiver){
    return db.query(`UPDATE friendships SET accepted = true WHERE receiver_id=${receiver} AND sender_id=${sender}`);
};
exports.cancelFriendshipRequest = function (sender, receiver){
    return db.query(`DELETE FROM friendships WHERE sender_id=${sender} AND receiver_id=${receiver}`);
};
exports.deleteFriendship = function (firstId,secondId){
    return db.query(`DELETE FROM friendships WHERE (receiver_id=${firstId} AND sender_id=${secondId}) OR (receiver_id=${secondId} AND sender_id=${firstId})`);
};
...
exports.addChatMessage = function (id,text){
    return db.query(`INSERT INTO chatMessages (sender_id,message) Values ($1,$2) RETURNING id AS messageId, sender_id AS usersId, created_at`,[id,text]);
};
exports.getUsersInfo =  function (arr) {
    return db.query(`SELECT id, avatarUrl, first,last FROM users WHERE id= ANY($1)`,[arr]);
};
...
exports.addPrivateMessage = function (receiver, sender, text){
    return db.query(`INSERT INTO privateMessages (receiver_id, sender_id, message) VALUES ($1, $2, $3) RETURNING id, created_at`,[receiver, sender, text]);
};
exports.pmUnseen = function (id, bool){
    return db.query(`UPDATE privateMessages SET seen = '${bool}' WHERE id = ${id}`);
};

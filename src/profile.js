    ...
    return (
        <div className="profile">
            ...
            <div className="editors">
                <BioEditor bio={props.bio} updateProfile={props.updateProfile}/>
                <InstrumentEditor instrument={props.instrument} updateProfile={props.updateProfile} />
                <BandEditor band={props.band} updateProfile={props.updateProfile}/>
            </div>
        </div>
    );
}
export class OtherUsersProfiles extends React.Component {
    ...
    render (){
        if (!this.state.validId){
            return ( <div className="error"> <h2>We could not find the requested profile, sorry!</h2></div>);
        }
        return (
            <div className="profile">
                <div>
                    <Avatar last={this.state.last}
                ...
                <div className="editors">
                    <div>
                        <div>
                            {this.state.bio && <div> <h4> bio: </h4> <p> {this.state.bio} </p></div> }
                            {this.state.instrument&& <div> <h4> Instruments: </h4> <p> {this.state.instrument} </p></div>}
                            {this.state.band && <div> <h4> band: </h4> <p> {this.state.band} </p></div>}
                        </div>
                    </div>
                </div>
            </div>
...
export function FriendButton (props){
    ...
    useEffect (()=>{
        instance.get(`/friendshipStatus/${props.id}`).then(({data})=>{
    ...
    function updateStatus(){
        instance.post(`/friendshipStatus`,{buttonMode:buttonMode,id:props.id}).then(({data})=>{
            setButtonMode(data.buttonMode);
        }).catch(err=>{

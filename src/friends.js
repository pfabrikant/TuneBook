...
export function Friends() {
    ...
    const loggedInUserDetails = useSelector(
        state => state && state.loggedInUserDetails
    );
    const friends = useSelector(state => {
        return (
            ...
            state.friends.filter(
                friend => friend.accepted && friend.id != loggedInUserDetails.id
            )
        );
    });
    const ignorers = useSelector(state => {
        return (
            ...
            state.friends.filter(
                friend =>
                    friend.sender_id == loggedInUserDetails.id &&
                    friend.receiver_id == friend.id &&
                    !friend.accepted
            )
        );
    });
    ...
    async function openMessageContainer(id, first, last, url) {
        ...
        privateMessagesRef.current &&
            privateMessagesRef.current.scrollIntoView({ behavior: "smooth" });
    }
    function sendMessage() {
        socket.emit("newPm", {
            receiver_id: messanger.id,
            sender_id: loggedInUserDetails.id,
            message: val,
            avatarurl: loggedInUserDetails.avatarurl,
            first: loggedInUserDetails.first,
            last: loggedInUserDetails.last
        });
    ...
    return (
        <div>
                ...
                {requesters &&
                    requesters.map(requester => (
                        <div key={requester.id}>
                            ...
                            <button
                                onClick={() =>
                                    dispatch(acceptFriend(requester.id))
                                }
                            >
                                Accept Friendship
                            </button>
            ...
            {ignorers && ignorers.length > 0 && (
                <h2> Friend requests sent to: </h2>
            )}
            {ignorers && ignorers.length == 0 && (
                <h2> You have not sent out any friend requests yet </h2>
            )}
            <div className="friends">
                {ignorers &&
                    ignorers.map(ignorer => (
                        <div key={ignorer.id}>
                            <Link
                                className="memberlinks"
                                to={"/users/" + ignorer.id}
                            ...
                            >
                                <div className="friend">
                                    <img
                                        src={ignorer.avatarurl}
                                        alt={ignorer.first + " " + ignorer.last}
                                    />
                                    <h3>
                                        {" "}
                                        {ignorer.first +
                            ...
                            <button
                                onClick={() =>
                                    dispatch(cancelRequest(ignorer.id))
                                }
                            >
                                Cancel Request
                            </button>
            ...
            <div>
                {messanger && messanger != null && (
                    <React.Fragment>
                        ...
                        <h3>
                            {" "}
                            My private conversation with{" "}
                            {messanger.first + " " + messanger.last}
                        </h3>

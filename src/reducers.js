export function reducer (state={}, action){
    ...
    if (action.type=="UPDATE_CHAT_MESSAGES"){
        return {
            ...state, chatMessages: action.data.reverse()
        };
    }
    ...
    if (action.type=="ADD_NEW_ONLINE_USER"){
        return { ...state,
            onlineUsers: [... state.onlineUsers, action.data]
        };
    }
    if (action.type=="OPEN_MESSANGER"){
        return { ...state, openMessanger:{id:action.id, first:action.first, last:action.last, avatarurl:action.url}};
    }
    ...
    if (action.type=="UPDATE_LOGGED_IN_USER_DETAILS"){
        return {
            ...state, loggedInUserDetails:action.data
        };
    }
    if (action.type=="UPDATE_NEW_PM"){
        ...
        return {
            ...state, privateMessages: [...state.privateMessages, action.data ]
        };
    }
    return {};
